...
**Assignments (Python)**: A1 simulate process state transitions; A2 `multiprocessing` vs `threading` benchmark (CPU-bound vs I/O-bound — observe GIL); A3 fork bomb explanation + safe demo.
**Exit criteria**:
- [ ] Process state diagram from memory
- [ ] Zombie vs orphan — kaise bante, kaun reap karta
- [ ] GIL kya hai, kab threading useless hai Python mein
...
**Assignments (Python)**: A1 scheduler simulator — given processes (arrival, burst) compute Gantt + avg waiting/turnaround for FCFS/SJF/RR (stub + tests); A2 add priority + aging.
**Exit criteria**:
...
**Assignments (Python)**: A1 demonstrate a race condition with threads + fix with `Lock`; A2 implement a counting semaphore from a Lock + Condition (stub + test); A3 bounded buffer with `Condition`.
**Exit criteria**:
...
**Assignments (Python)**: A1 producer–consumer with `queue.Queue` then from scratch with Condition (stub + passing test: no lost/dup items); A2 dining philosophers — show deadlock, then fix with ordering (must not deadlock under stress test).
**Exit criteria**:
...
**Assignments (Python)**: A1 detect deadlock via cycle detection in RAG (stub + tests); A2 Banker's algorithm safety check (stub + test cases safe/unsafe).
**Exit criteria**:
...
**Assignments (Python)**: A1 logical→physical address translation given page size + page table (stub + tests); A2 EAT calculator given TLB hit ratio + access times.
**Exit criteria**:
...
**Assignments (Python)**: A1 page replacement simulator — FIFO/LRU/Optimal, count faults (stub + tests, show Belady on FIFO); A2 working-set window computation.
**Exit criteria**:
...
**Assignments (Python)**: A1 compute max file size from inode pointer scheme; A2 simulate a simple inode-based file lookup (stub).
**Exit criteria**:
...
**Assignments (Python)**: A1 disk scheduling simulator — head movement for each algorithm (stub + tests).
**Exit criteria**:
...
**Assignments (Python)**: A1 producer/consumer across processes via `multiprocessing.Queue`/`Pipe`; A2 signal handler demo (graceful shutdown).
**Exit criteria**:
...
**Topics**: `/proc` & `/sys`; `top`/`htop`/`ps`/`vmstat`/`iostat` reading; `strace`/`ltrace`; load average meaning; OOM killer; cgroups & namespaces (→ containers/Docker bridge); file descriptors & `lsof`; `nice`/`renice`. **Rapid-fire FAQ**: process vs thread, deadlock, paging vs segmentation, virtual memory, context switch cost, what happens on `malloc`, mutex vs semaphore, GIL, fork vs exec.
**Assignments**: A1 read a `top` snapshot — explain CPU/mem/load; A2 use `strace` on a Python script, identify syscalls.
**Exit criteria**:
...
| Wed–Thu | Assignment implementation (Python) |
