# Machine Coding Problems — Timed Python Practice 🔥

> Har problem: `README.md` (requirements + time budget + passing criteria) + `solution.py` (starter stub). **Timer laga ke** solve karo, demo/asserts pass karao, phir rubric pe self-score.
...
4. `python solution.py` → asserts must pass.
...
|---|---------|--------|----------|--------|
| 1 | LRU Cache | 30m | dict + DLL / OrderedDict | `lru-cache/` |
| 2 | LFU Cache | 45m | freq buckets | `lfu-cache/` |
| 3 | Rate Limiter | 30m | token bucket (CV) | `rate-limiter/` |
| 4 | In-memory KV store | 45m | dict + TTL + txn | `in-memory-kv-store/` |
| 5 | In-memory SQL | 60m | table + where/order/limit | `in-memory-sql/` |
| 6 | Task Scheduler | 45m | heap by run-time | `task-scheduler/` |
...
| 12 | Notification Service | 45m | channels + retry + dedup (CV: outbox) | `notification-service/` |

> Ramp: 1 LRU → 3 Rate limiter → 7 Logging → 11 Tic-Tac-Toe → 4 KV → 6 Scheduler → 8 Calendar → 5 SQL → 9 File system.
