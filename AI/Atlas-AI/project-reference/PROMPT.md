...
- evolve from a simple zero-cost local baseline to production deployment only when measurements justify added complexity.
...
- Build, test, and demo paths must require zero monetary cost. Paid SaaS, cloud resources, domains, and paid model APIs are optional integrations only and must never run or provision without explicit approval.
- Deterministic fakes/mocks or genuinely free local alternatives are required for provider-facing tests and demos. Large local LLM downloads must not be mandatory.
...
- deterministic local model/provider fakes for tests and demos, plus optional OpenAI and/or equivalent model providers through adapters
- OpenTelemetry
- optional Langfuse or another AI observability sink behind an adapter
- Docker
- Terraform for explicit opt-in infrastructure planning/provisioning
- optional AWS-oriented production deployment
- GitHub Actions
...
- demonstrate the zero-cost local path and keep any billable provider/cloud integration disabled unless explicitly approved;
